...
import v14.helper.KioskIOHandler;
import v14.repository.MenuRepository;
import v14.utils.FunctionalExample;
import v14.utils.KioskUtils;
...
//            menus.stream().forEach((menu -> ioHandler.writeOutput(menu.getName() + "(" + menu.getPrice()+"원)")));
...
//        return menuRepository.findById(menuNumber);
    ...
    // 특정 조건에 따라 메뉴를 필터링하는 메서드를 추가
    // 우리는 가격을 입력받아서, 이 가격으로 살 수 있는 상품만 리턴하는 메서드를 정의
    private List<Menu> filterMenuByPrice(int maxPrice) {
        List<Menu> menus = menuRepository.findAll();

        // 최대 금액 이하의 메뉴만 필터링
        List<Menu> filteredMenus = FunctionalExample.filterMenus(menus,
                menu -> menu.getPrice() <= maxPrice);

        return filteredMenus;
    }

    private List<Menu> filterMenuBySaleStatus(SaleStatus status) {
        List<Menu> menus = menuRepository.findAll();
        List<Menu> filteredMenus = FunctionalExample.filterMenus(menus,
                menu -> menu.getSaleStatus() == status);
        return filteredMenus;
    }
}